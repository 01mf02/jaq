...
pub fn from_iso8601(s: &str) -> ValR {
    ...
    let datetime = OffsetDateTime::parse(s, &Iso8601::DEFAULT)
        .map_err(|e| Error::str(format!("cannot parse {s} as ISO-8601 timestamp: {e}")))?;
...
pub fn to_iso8601(v: &Val) -> Result<String, Error> {
    ...
    let fai1 = |e| Error::str(format!("cannot format {v} as ISO-8601 timestamp: {e}"));
    let fai2 = |e| Error::str(format!("cannot format {v} as ISO-8601 timestamp: {e}"));
