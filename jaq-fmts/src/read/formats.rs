...
pub fn read_string(fmt: Format, read: impl Read) -> Result<String> {
    use Format::*;
    match fmt {
        Raw | Raw0 | Json | Jsonc | Json5 | Cbor => Ok(String::new()),
...
pub fn bytes_str(fmt: Format, bytes: &[u8]) -> Result<&str> {
    use Format::*;
    Ok(match fmt {
        Raw | Raw0 | Json | Jsonc | Json5 | Cbor => "",
...
pub fn read<'a>(fmt: Format, read: impl io::BufRead + 'a, s: &'a str, slurp: bool) -> Vals<'a> {
    ...
    match fmt {
        ...
        Format::Jsonc => collect_if(slurp, json::read_many_jsonc(read)),
        Format::Json5 => collect_if(slurp, json::read_many_json5(read)),
...
pub fn parse<'a>(fmt: Format, bytes: &'a Bytes, s: &'a str, slurp: bool) -> Vals<'a> {
    ...
    match fmt {
        ...
        Format::Jsonc => collect_if(slurp, json::parse_many_jsonc(bytes).map(map_invalid_data)),
        Format::Json5 => collect_if(slurp, json::parse_many_json5(bytes).map(map_invalid_data)),
