...
fn parse_int(s: &str) -> Option<Val> {
    ...
    let i = isize::from_str_radix(s, radix).ok()?;
    Some(Val::Int(if pos == Some('-') { -i } else { i }))
}
...
fn parse_float(s: &str) -> Option<Val> {
    ...
    if matches!(rest, ".inf" | ".Inf" | ".INF") {
        let f = f64::INFINITY;
        Some(Val::Float(if sign == Some('-') { -f } else { f }))
    } else {
        normalise_float(sign, rest).map(|f| Val::Num(f.into()))
    }
}
