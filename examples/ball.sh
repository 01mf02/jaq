...
$JQ -nc -L . -n 'include "ball"; {x: 50, y: 200, r: 25, col: [0, 255, 0], vx: 2.5, vy: 0} | animate_ball(96)' | while read line
do
  #echo $line
  file=ball/$(printf "%02d" $i).ppm
  echo $file
  echo $line | $JQ -L . --to binary 'include "ball"; 320 as $w | 240 as $h | [ppm_raw($w; $h), render_ball($w; $h)] | tobytes' > $file
  ((i++))
done
